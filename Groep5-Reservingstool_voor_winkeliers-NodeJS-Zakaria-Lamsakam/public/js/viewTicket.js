...
window.onload = () => {
    ...
    getUser().then(user => {
        ...
        async function loadTicket() {
            let ticket = await getData(`../../api/get/tickets?id=${id}&customer_id=${user.id}`);
            ...
            if (ticket[0].status == 1) {
                ...
                statusButton.addEventListener('click', (event) => {
                    getData(`../../api/update/tickets?set_status=0&id=${id}&customer_id=${user.id}`);
                ...
                form.addEventListener('submit', (event) => {
                    ...
                    getData(`../../api/insert/ticket_messages?ticket_id=${id}&customer_id=${user.id}&message=${messageArea.value}`);
                    window.location.href = `?id=${id}`;
                });
            }
            else {
                ...
                statusButton.addEventListener('click', (event) => {
                    getData(`../../api/update/tickets?set_status=1&id=${id}&customer_id=${user.id}`);
                    window.location.href = `?id=${id}`;
                });
