...
window.onload = () => {
    getUser().then(user => {
        ...
        async function insertTicket() {
            let ticketResult = await getData(`../../api/insert/tickets?customer_id=${user.id}&title=${encodeURIComponent(inputTitle.value)}`);
            console.log("if 1");
            ...
            let ticketResultAfterInserted = await getData(`../../api/get/tickets?customer_id=${user.id}&order_by_desc=id&sql_limit=1`);
            console.log("if 2");
            ...
            let ticketMessageResult = await getData(`../../api/insert/ticket_messages?ticket_id=${ticketResultAfterInserted[0].id}&customer_id=${user.id}&message=${encodeURIComponent(inputDescription.value)}`);
            console.log("if 3");
